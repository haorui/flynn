...
func scanDeployment(s postgres.Scanner) (*ct.Deployment, error) {
	d := &ct.Deployment{}
	err := s.Scan(&d.ID, &d.AppID, &d.OldReleaseID, &d.NewReleaseID, &d.Strategy, &d.CreatedAt)
...
func streamDeploymentEvents(w http.ResponseWriter, req *http.Request, params martini.Params, repo *DeploymentRepo) {
